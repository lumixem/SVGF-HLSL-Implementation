// Based on NVIDIA's Falcor SVGF Pass
...
ConstantBuffer<StepSize> stepSize : register(b2); // Step size for A-Trous wavelet filter. See figure 4 in paper
...
// Gaussian blur for blurring the variance
...
void main(uint3 idx : SV_DispatchThreadID)
{
    ...
    // Check whether we git any geometry
    if (depthBuffer[idx.x] >= 0.f)
    {
        // Calculate edge stopping functions
        ...
        float phiDepth = stepSize.m_Value; // We don't use depth derivates as the paper mentions as we didn't notice any visual improvement
