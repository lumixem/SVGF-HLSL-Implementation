...
StructuredBuffer<float4> intersectionPoints : register(t0); // World space intersection points
...
StructuredBuffer<float4> frameColor : register(t8); // 1spp input 
StructuredBuffer<float4> primaryAlbedo : register(t9); // albedo at the primary ray intersection point
StructuredBuffer<float4> emission : register(t10); // emission used here for demodulation
StructuredBuffer<CameraGPU> cameras : register(t11); // array of [previous frame camera, current frame camera]
...
// Check whether reprojection is possible
...
void main(uint3 idx : SV_DispatchThreadID)
{
    ...
    // Albedo demodulation to preserve texture details. See section 3 - Reconstruction Pipeline in paper
    ...
    // prevPixelIdx written to this buffer in an earlier ReSTIR reprojection shader
    ...
    if (prevPixelIdx >= 0)
    {
        ...
        // Bilinear check of previous pixels. See section 4.1 - Temporal filtering in paper
        // Check whether we have a single useful pixel from the previous frame, using bilinear 2x2 filter
        ...
        // Reprojection succeeded and we can interpolate data from the current frame with the previous frame
        if (successBilin)
        {
            float colorAlpha = max(1.0f / (prevAccumFrames + 1.f), settings.m_Alpha);
            historyUpdate[idx.x] = uint(prevAccumFrames) + 1;
    ...
    // If we didn't hit geometry set the pixel's frame history to 1
